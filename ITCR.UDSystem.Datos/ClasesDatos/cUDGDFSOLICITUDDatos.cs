...
namespace ITCR.UDSystem.Datos
{
    ...
    public class cUDGDFSOLICITUDDatos : cUDGDFSOLICITUDBase
    {
        ...
        /// <returns>DataTable Object</returns>
        public virtual DataTable SeleccionarAprobadas(DateTime p_fechainicio, DateTime p_fechafin)
        {
            SqlCommand cmdAEjecutar = new SqlCommand();
            cmdAEjecutar.CommandText = "dbo.[pr_UDGDFSOLICITUD_Seleccionar_Aprobadas]";
            cmdAEjecutar.CommandType = CommandType.StoredProcedure;
            DataTable toReturn = new DataTable("UDGDFSOLICITUD");
            SqlDataAdapter adapter = new SqlDataAdapter(cmdAEjecutar);
            ...
            try
            {
                ...
                {
                    // Genera un error.
                    throw new Exception("Procedimiento Almacenado 'pr_UDGDFSOLICITUD_Seleccionar_Aprobadas' reportó el error Código: " + _codError);
                }
            ...
            {
                // Ocurrió un error. le hace Bubble a quien llama y encapsula el objeto Exception
                throw new Exception("cUDGDFSOLICITUDDatos::SeleccionarAprobadas::Ocurrió un error." + ex.Message, ex);
            }
            finally
            {
                if (_conexionBDEsCreadaLocal)
